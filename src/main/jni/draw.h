...
std::string GetKeyGeneratorLink(const std::string& uuid) {
    ...
    return std::string(oxorany("https://bskey.replit.app/?hwid=")) + uuid + oxorany("&signature=") + signature;
}
...
bool VerifyKeyWithServer(const std::string& key, const std::string& uuid) {
    ...
    std::string url = oxorany("https://bskey.replit.app/verify?key=");
    url += escaped_key;
    url += oxorany("&hwid=");
    url += escaped_uuid;
...
void DrawMenu(ImGuiIO &io) {
    ...
    if (ImGui::Begin("##SACREDBS_MENU", &showMenu, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse)) {
        ...
        ImGui::BeginChild("##MainContent", ImVec2(0, ImGui::GetContentRegionAvail().y - 6.0f), false, ImGuiWindowFlags_AlwaysVerticalScrollbar);
