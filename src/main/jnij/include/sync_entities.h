...
struct Entity {
  ...
  //  bool is_snap = false;
    ...
    float is_health = 123.0f;   // Bisa berisi hasil scaling atau clamped
    float is_armor = 125.0f;    // Bisa digunakan untuk bar ESPfloat
	float is_team_id = 0;
};
        ...
        try {
            ...
            while (std::getline(ss, line)) {
                ...
                for (const auto& entity_json : entities_json) {
                  ...
                  /*  if (entity_json["is_snap"].is_boolean()) {
    entity.is_snap = entity_json["is_snap"].get<bool>();
}*/
...
if (entity_json.contains("is_armor") && entity_json["is_armor"].is_number()) {
    entity.is_armor = entity_json["is_armor"].get<float>(); // Simpan ke armor, bukan ke health
}
if (entity_json.contains("is_team_id") && entity_json["is_team_id"].is_number()) {
    entity.is_team_id = entity_json["is_team_id"].get<float>(); // Simpan ke armor, bukan ke health
}
                    {
                        std::lock_guard<std::mutex> lock(entities_mutex);
