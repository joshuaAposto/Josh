...
std::string GetKeyGeneratorLink(const std::string& uuid) {
    ...
    return std::string(oxorany("https://ryukobs-generator.up.railway.app/?hwid=")) + uuid + oxorany("&signature=") + signature;
}
    ...
    if (res == CURLE_OK) {
        try {
            ...
            if (data.contains(oxorany("success")) && data[oxorany("success")].is_boolean()) {
                ...
                if (!isSuccess) {
                    snprintf(auth_status, sizeof(auth_status), oxorany("Login Failed: Invalid or Expired Key!"));
                } else {
...
void DrawMenu(ImGuiIO &io) {
    ...
    ImVec4 themeColor = ImVec4(0.9f, 0.5f, 0.9f, 1.0f); 
    ...
    if (!showMenu) {
        ...
        if (ImGui::Begin("##OpenMenuBtn", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings)) {
            ImGui::SetWindowFontScale(1.20f); 
            const char* r_text = oxorany("R");
            auto textSize = ImGui::CalcTextSize(r_text);
            ImGui::SetCursorPos(ImVec2((60.0f - textSize.x) / 2.0f, (60.0f - textSize.y) / 2.0f)); 
            ImGui::TextColored(themeColor, "%s", r_text);
            ...
            if (ImGui::IsWindowHovered()) {
                ...
                if (ImGui::IsMouseReleased(0)) {
                    ...
                    if (abs(clickPos.x - releasePos.x) < 5.0f && abs(clickPos.y - releasePos.y) < 5.0f) {
                        showMenu = true;
                    }
                }
            }
    ...
    if (auth_state.load() != AUTH_SUCCESS) {
        ...
        if (ImGui::Begin("##KeySystem", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar)) {
            ...
            const char* title = oxorany("RYUKOBS - LOGIN");
            ...
            ImGui::TextDisabled(oxorany("DEVICE INFO:"));
            ImGui::Text(oxorany("Model: %s"), getDeviceName().c_str());
            ImGui::Text(oxorany("UUID: %s"), cached_uuid.c_str());
            ...
            if (ImGui::Button(oxorany("PASTE KEY FROM CLIPBOARD"), ImVec2(-1, 40))) {
                ...
                if (!clip.empty()) {
                    ...
                    snprintf(auth_status, sizeof(auth_status), oxorany("Key pasted successfully!"));
                } else {
            ...
            if (is_authenticating) {
                ...
                ImGui::Button(oxorany("VERIFYING..."), ImVec2(-1, 50));
                ImGui::PopStyleVar();
            } else {
                if (ImGui::Button(oxorany("LOGIN"), ImVec2(-1, 50))) {
            ...
            ImGui::TextColored(ImColor(255, 50, 50), "%s", auth_status);
        }
        ImGui::End();
        ImGui::PopStyleVar();
        return; 
    ...
    if (ImGui::Begin("##SACHI_VVIP", &showMenu, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse)) {
        
        ImGui::BeginGroup();
        {
            ImGui::SetCursorPos(ImVec2(15, ImGui::GetCursorPosY() + 2)); 
            ...
            int r = (time < 2.0f) ? (int)(sin(time * 3.0f) * 127 + 128) : 238;
            int g = (time < 2.0f) ? (int)(sin(time * 3.0f + 3.0f) * 127 + 128) : 130;
            int b = (time < 2.0f) ? (int)(sin(time * 3.0f + 4.0f) * 127 + 128) : 238;
            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(r, g, b, 255));
            if (ImGui::Button(oxorany("RyukoBS Freemium 650015 - Click here to minimize"))) showMenu = false;
        ...
        auto NavBtn = [&](const char* label, int id) {
            ...
            if (is_active) ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.9f, 0.5f, 0.9f, 0.45f));
            if (ImGui::Button(label, ImVec2(btnWidth, 40))) { tab = id; tab_changed = true; }
        ...
        if (tab == 5) { 
            if (ImGui::BeginTable("##I_T", 2, ImGuiTableFlags_SizingStretchProp)) {
                ...
                ImGui::Separator(); ImGui::Spacing();
                ImGui::Text(oxorany("DEVELOPER : RYUKO"));
                ImGui::Spacing();
                ImGui::Text(oxorany("OFFICIAL TELEGRAM : https://t.me/ryukobs"));
                ImGui::Spacing(); ImGui::Spacing();
