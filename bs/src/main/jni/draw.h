...
if (!sync_bool["bAIM_NoTargetHideFov"] || (players || bots)) {
    ...
    if (!isLogin) {
        if (ImGui::Begin(xorstr_("LOGIN PAGE"), NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize)) {
            ImGui::Text(xorstr_(ICON_FA_USER_SECRET " UCIHA MADARA PANEL"));
            ImGui::Separator();
            ImGui::Spacing();
            ImGui::Text(xorstr_("Contact @haxkrafton to buy license"));
            ImGui::Spacing();
            
            static char s[150] = {0};
            ImGui::InputTextWithHint(xorstr_("##key"), xorstr_("Enter License Key"), s, sizeof(s));
            ...
            if (ImGui::Button(xorstr_("LOGIN"), ImVec2(ImGui::GetWindowContentRegionWidth(), 0))) {
                // ✅ VALIDASI MENGGUNAKAN API PANEL
                err = xorstr_("Validating...");
                
                if (g_env && g_context) {
                    LOGI("[LICENSE] Checking license: %s", s);
                    
                    bool isValid = ValidateLicense(g_env, g_context, s);
                    
                    if (isValid) {
                        err = xorstr_("OK");
                        isLogin = true;
                        xxxxx = true;
                        LOGI("[LICENSE] ✅ License Valid!");
                    } else {
                        err = xorstr_("Invalid License Key!");
                        isLogin = false;
                        LOGI("[LICENSE] ❌ License Invalid!");
                    }
                } else {
                    err = xorstr_("System Error: JNI not initialized!");
                    LOGI("[LICENSE] ❌ JNI Error!");
                }
            }

            if (!err.empty() && err != std::string(xorstr_("OK"))) {
                ImGui::Spacing();
                ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), xorstr_("ERROR: %s"), err.c_str());
            }
    ...
    if (ImGui::Begin(xorstr_("LOGIN PAGE"), NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize)) {
        ...
        if (ImGui::Button(xorstr_("LOGIN"), ImVec2(ImGui::GetWindowContentRegionWidth(), 0))) {
            ...
            if (err == "OK" && (bValid && g_Auth == g_Token)) {
                isLogin = true;
                xxxxx = true;  // UI hanya muncul setelah tombol LOGIN ditekan dan valid
            }
        }
	...
	*/
    ...
    // ✅ FLOATING BUTTON
    {
        ...
        ImGui::Begin(xorstr_("##FloatingBtn"), nullptr, 
            ImGuiWindowFlags_NoTitleBar | 
            ImGuiWindowFlags_NoResize | 
            ImGuiWindowFlags_NoScrollbar |
            ImGuiWindowFlags_NoBackground);
        {
            ImDrawList* floatDraw = ImGui::GetWindowDrawList();
            ImVec2 btnPos = ImGui::GetWindowPos();
            
            floatDraw->AddCircleFilled(ImVec2(btnPos.x + 40, btnPos.y + 40), 35.0f, ImColor(3, 169, 245, 255), 100);
            floatDraw->AddCircle(ImVec2(btnPos.x + 40, btnPos.y + 40), 35.0f, ImColor(255, 255, 255, 100), 100, 2.0f);
            
            ImGui::PushFont(TELEGRAM);
            floatDraw->AddText(TELEGRAM, 40.f, ImVec2(btnPos.x + 18, btnPos.y + 18), ImColor(255, 255, 255, 255), xorstr_("Show"));
            ImGui::PopFont();
            
            ImGui::SetCursorPos({5, 5});
            if(ImGui::InvisibleButton(xorstr_("##toggleMenu"), ImVec2(70, 70))) {
                showMenu = !showMenu;
            }
            
            if(ImGui::IsItemHovered()) {
                floatDraw->AddCircle(ImVec2(btnPos.x + 40, btnPos.y + 40), 37.0f, ImColor(255, 255, 255, 200), 100, 2.5f);
            }
        }
        ImGui::End();
    }
    
    // ✅ RETURN JIKA MENU DITUTUP
    if(!showMenu) return;
    if(!itsRohitOp) return;
    
    ImGui::SetNextWindowSize(ImVec2(1000, 680)); // 680
    ImGui::MainBegin(xorstr_("Menu"), nullptr, ImGuiWindowFlags_::ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
    {
        ImVec2 P1, P2;
        ...
        pBackgroundDrawList->AddRectFilled(ImVec2(0.000f + p.x, 0.000f + p.y), ImVec2(1000 + p.x, 680 + p.y), ImColor(15,17,16,255), 10);
        
        static float fps = 0.0f;
        fps = ImGui::GetIO().Framerate;
        char fpsText[32];
        snprintf(fpsText, sizeof(fpsText), xorstr_("FPS: %.3f"), fps);
        
        draw->AddRectFilled(ImVec2(pos.x + 15, pos.y + 15), ImVec2(pos.x + 115, pos.y + 665), ImColor(28, 30, 29, 255), 10.f, ImDrawFlags_RoundCornersAll);
        draw->AddRectFilled(ImVec2(pos.x + 130, pos.y + 15), ImVec2(pos.x + 985, pos.y + 190), ImColor(28, 30, 29, 255), 10.f, ImDrawFlags_RoundCornersAll);
        draw->AddRect(ImVec2(pos.x + 15, pos.y + 15), ImVec2(pos.x + 115, pos.y + 665), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        draw->AddRect(ImVec2(pos.x + 130, pos.y + 15), ImVec2(pos.x + 985, pos.y + 190), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        draw->AddRect(ImVec2(pos.x + 520, pos.y + 25), ImVec2(pos.x + 975, pos.y + 100), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        
        draw->AddText(F50, 45.f, ImVec2(530 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("Menu"));
        draw->AddText(Subtab, 35.f, ImVec2(770 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), fpsText);
        
        draw->AddRect(ImVec2(pos.x + 140, pos.y + 120), ImVec2(pos.x + 740, pos.y + 180), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        draw->AddRect(ImVec2(pos.x + 750, pos.y + 120), ImVec2(pos.x + 975, pos.y + 180), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        draw->AddRect(ImVec2(pos.x + 30, pos.y + 580), ImVec2(pos.x + 100, pos.y + 650), ImColor(100, 100, 100, 255), 10.f, ImDrawFlags_RoundCornersAll, 2.0f);
        draw->AddLine(ImVec2(pos.x + 30, pos.y + 140), ImVec2(pos.x + 100, pos.y + 140), ImColor(100, 100, 100, 255), 2.f);
        
        draw->AddCircleFilled(ImVec2(pos.x + 65, pos.y + 77.5), 36.0f, ImColor(3, 169, 245, 255), 100.0f);
        draw->AddText(TELEGRAM, 45.f, ImVec2(38 + p.x, 55 + p.y), ImColor(255, 255, 255, 255), xorstr_("Nexusdev"));
        draw->AddCircleFilled(ImVec2(pos.x + 185, pos.y + 70.0), 40.0f, ImColor(100, 100, 100, 255), 100.0f);
        draw->AddCircleFilled(ImVec2(pos.x + 185, pos.y + 70.0), 38.0f, ImColor(60, 60, 60, 255), 100.0f);
        
        ImGui::SetCursorPos({38, 50});
        if(ImGui::OptButton(xorstr_(""), ImVec2(40, 40), true)) {
        }
        
        ImGui::PushFont(logoicon);
        ImGui::SetCursorPos({35, 580}); // 580
        if(ImGui::OptButton0(xorstr_("X"), ImVec2(60, 60), true)) {
            showMenu = false;
        }
        ImGui::PopFont();
        ...
        ImGui::SetCursorPos(ImVec2(32,155)); // 155
        ImGui::BeginGroup();
        {
            ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {9, 15});
            if (ImGui::ButtonExes1(xorstr_("Esp"), ActiveTab != 1)) { ActiveTab = 1; }
            if (ImGui::ButtonExes1(xorstr_("Aim"), ActiveTab != 2)) { ActiveTab = 2; }
            if (ImGui::ButtonExes1(xorstr_("SH"), ActiveTab != 3)) { ActiveTab = 3; }
            if (ImGui::ButtonExes1(xorstr_("WH"), ActiveTab != 4)) { ActiveTab = 4; }
            if (ImGui::ButtonExes1(xorstr_("Info"), ActiveTab != 5)) { ActiveTab = 5; }
            // ✅ TAMBAHAN: Tab baru untuk Anti-Telemetry
            if (ImGui::ButtonExes1(xorstr_("🛡"), ActiveTab != 6)) { ActiveTab = 6; }
            ImGui::PopStyleVar();
        }
        ImGui::EndGroup();
        
        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {14, 9});
        
        // ========== TAB 1: VISUALS ==========
        if (ActiveTab == 1) {
            draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("VISUALS"));
            draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("Esp"));
            draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("ESP fully safe!! (Use All Features)"));
            
            ImGui::SetCursorPos(ImVec2(250,67));
            ImGui::BeginGroup();
            {
                if (ImGui::ButtonExes2(xorstr_("Main"), ActiveSubTab != 1)) { ActiveSubTab = 1; }
                ImGui::SameLine();
                if (ImGui::ButtonExes2(xorstr_("Advanced"), ActiveSubTab != 2)) { ActiveSubTab = 2; }
            }
            ImGui::EndGroup();
            
            if (ActiveSubTab == 1) {
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("ESP-Main-1"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                    ...
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("LINE POSITION"));
                ...
                ImGui::SetCursorPos(ImVec2(562.5, 195));
                ImGui::MenuChild(xorstr_("ESP-Main-2"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                ...
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("ESP-Adv"), ImVec2(855, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("COLOR SETTINGS"));
                    ImGui::Spacing();
        ...
        // ========== TAB 2: COMBAT ==========
            ...
            draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("COMBAT"));
            draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("Aim"));
            draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("Aimbot & Combat Features"));
            
            ImGui::SetCursorPos(ImVec2(250,67));
            ImGui::BeginGroup();
            {
                if (ImGui::ButtonExes2(xorstr_("Aimbot"), ActiveSubTab != 1)) { ActiveSubTab = 1; }
                ImGui::SameLine();
                if (ImGui::ButtonExes2(xorstr_("Memory"), ActiveSubTab != 2)) { ActiveSubTab = 2; }
            }
            ImGui::EndGroup();
            
            if (ActiveSubTab == 1) {
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("Aim-1"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("AIMBOT MEMORY"));
                    ImGui::Separator();
                    ...
                    ImGui::Text(xorstr_("BULLET TRACK"));
                    ImGui::Separator();
                    ...
                    ImGui::Text(xorstr_("LOCK-ON AIMBOT"));
                    ImGui::Separator();
                ...
                ImGui::SetCursorPos(ImVec2(562.5, 195));
                ImGui::MenuChild(xorstr_("Aim-2"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("FOV SETTINGS"));
                    ImGui::Separator();
                    ...
                    ImGui::Text(xorstr_("AIM SETTINGS"));
                    ImGui::Separator();
                    ...
                    need_sync |= SliderFloat(xorstr_("STRENGTH"), &sync_float[xorstr_("fAIM_SnapStrength")], 0.001f, 3.0f, xorstr_("%.2f"));
                }
                ImGui::EndChild();
            }
            else if (ActiveSubTab == 2) {
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("Memory-1"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("RECOIL & SPREAD"));
                    ImGui::Separator();
                    ...
                    ImGui::Text(xorstr_("MAGIC BULLET"));
                    ImGui::Separator();
                ...
                ImGui::SetCursorPos(ImVec2(562.5, 195));
                ImGui::MenuChild(xorstr_("Memory-2"), ImVec2(422.5, 470));
                {
                    ImGui::Spacing();
                    ...
                    Toggle(xorstr_("PREDICTION"), &lockPrediction);
                    if (lockPrediction) {
                        ImGui::SliderFloat(xorstr_("PREDICT STRENGTH"), &predictionStrength, 0.5f, 100.0f, xorstr_("%.1f"));        
                    }
                }
                ImGui::EndChild();
            }
        ...
        // ========== TAB 3: MOVEMENT ==========
            ...
            draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("MOVEMENT"));
            draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("SH"));
            draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("Speed & Movement Hacks"));
            
            ImGui::SetCursorPos(ImVec2(130, 195));
            ImGui::MenuChild(xorstr_("Movement-1"), ImVec2(855, 470));
            {
                ImGui::Spacing();
                ImGui::Text(xorstr_("SPEED HACK"));
                ImGui::Separator();
        ...
        // ========== TAB 4: OTHER ==========
            ...
            draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("OTHER"));
            draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("WH"));
            draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("Xray, Chams & Other Features"));
            
            ImGui::SetCursorPos(ImVec2(250,67));
            ImGui::BeginGroup();
            {
                if (ImGui::ButtonExes2(xorstr_("Xray"), ActiveSubTab != 1)) { ActiveSubTab = 1; }
                ImGui::SameLine();
                if (ImGui::ButtonExes2(xorstr_("Render"), ActiveSubTab != 2)) { ActiveSubTab = 2; }
            }
            ImGui::EndGroup();
            
            if (ActiveSubTab == 1) {
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("Xray-1"), ImVec2(855, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("XRAY SETTINGS"));
                    ImGui::Separator();
                ...
                ImGui::SetCursorPos(ImVec2(130, 195));
                ImGui::MenuChild(xorstr_("Render-1"), ImVec2(855, 470));
                {
                    ImGui::Spacing();
                    ImGui::Text(xorstr_("CHAMS SETTINGS"));
                    ImGui::Separator();
            ...
            draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("INFO"));
            draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("Info"));
            draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("Information & Credits"));
            
            ImGui::SetCursorPos(ImVec2(130, 195));
            ImGui::MenuChild(xorstr_("Info-1"), ImVec2(855, 470));
            {
                ImGui::Spacing();
                ImGui::TextColored(ImVec4(0.0f, 1.0f, 1.0f, 1.0f), xorstr_("MENU INFORMATION"));
                ImGui::Separator();
                ImGui::Spacing();
                
                ImGui::Text(xorstr_("Menu Name: United Front"));
                ImGui::Text(xorstr_("Version: 1.1.0"));
                ImGui::Text(xorstr_("Build Date: 2025"));
                
                ImGui::Spacing();
                ImGui::Separator();
                ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), xorstr_("FEATURES"));
                ImGui::Separator();
                ...
                ImGui::Spacing();
                ImGui::Separator();
                ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), xorstr_("CREDITS"));
                ImGui::Separator();
                ImGui::Text(xorstr_("Developer: NEXUSDEV009"));
                ImGui::BulletText(xorstr_("Telegram : @NEXUSDEV009"));
                ImGui::BulletText(xorstr_("Created  : January 2026"));
                ImGui::BulletText(xorstr_("Version  : 1.1.0"));
                ImGui::BulletText(xorstr_("Game     : BloodStrike"));
                ImGui::Text(xorstr_("Special Thanks to the UNITED FRONT"));
                
                ImGui::Spacing();
                ImGui::Separator();
                ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.5f, 0.0f, 1.0f));
                ImGui::TextWrapped(xorstr_("WARNING: Use at your own risk. This tool is for educational purposes only."));
                ImGui::PopStyleColor();
            }
            ImGui::EndChild();
        }
        
        // ========== TAB 6: ANTI-TELEMETRY ==========
        else if (ActiveTab == 6) {
            if (!globalAntiTelemetry) {
                draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 0, 0, 255), xorstr_("ERROR"));
                draw->AddText(Subtab, 25.f, ImVec2(160 + p.x, 134 + p.y), ImColor(255, 0, 0, 255), xorstr_("Anti-Telemetry not initialized!"));
            } else {
                draw->AddText(F50, 30.f, ImVec2(250 + p.x, 40 + p.y), ImColor(255, 255, 255, 255), xorstr_("ANTI-TELEMETRY"));
                draw->AddText(F48, 45.f, ImVec2(162.5 + p.x, 47.5 + p.y), ImColor(200, 200, 200, 255), xorstr_("G"));
                draw->AddText(Subtab, 35.f, ImVec2(160 + p.x, 134 + p.y), ImColor(200, 200, 200, 255), xorstr_("Protection & Statistics"));
                
                ImGui::SetCursorPos(ImVec2(250,67));
                ImGui::BeginGroup();
                {
                    if (ImGui::ButtonExes2(xorstr_("Protection"), ActiveSubTab != 1)) { ActiveSubTab = 1; }
                    ImGui::SameLine();
                    if (ImGui::ButtonExes2(xorstr_("Statistics"), ActiveSubTab != 2)) { ActiveSubTab = 2; }
                    ImGui::SameLine();
                    if (ImGui::ButtonExes2(xorstr_("Settings"), ActiveSubTab != 3)) { ActiveSubTab = 3; }
                }
                ImGui::EndGroup();
                
                // ========== SUB-TAB 1: PROTECTION ==========
                if (ActiveSubTab == 1) {
                    ...
                    ImGui::SetCursorPos(ImVec2(130, 195));
                    ImGui::MenuChild(xorstr_("AT-Prot-1"), ImVec2(422.5, 470));
                    {
                        ImGui::Spacing();
                        ...
                        if (enabled) {
                            ...
                            ImGui::Text(xorstr_("● ACTIVE"));
                            ImGui::PopStyleColor();
                        } else {
                            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.0f, 0.0f, 1.0f));
                            ImGui::Text(xorstr_("● DISABLED"));
                            ImGui::PopStyleColor();
                        }
                        
                        ImGui::Separator();
                        
                        int totalBlocked = globalAntiTelemetry->getTotalBlocked();
                        int uniqueBlocked = (int)globalAntiTelemetry->getBlockHistory().size();
                        
                        ImGui::Text(xorstr_("📊 Statistics:"));
                        ImGui::Indent(15.0f);
                        ImGui::Text("Total Blocked: %d", totalBlocked);
                        ImGui::Text("Unique Domains: %d", uniqueBlocked);
                        ...
                        if (ImGui::CollapsingHeader(xorstr_("✓ Protected Categories"), ImGuiTreeNodeFlags_DefaultOpen)) {
                            ImGui::Indent(10.0f);
                            ImGui::TextColored(ImVec4(0.5f, 1.0f, 0.5f, 1.0f), xorstr_("Currently Blocking:"));
                            ...
                            ImGui::BulletText(xorstr_("Crash reporting"));
                            ImGui::BulletText(xorstr_("Device fingerprinting"));
                            ImGui::BulletText(xorstr_("Third-party analytics"));
                            ImGui::BulletText(xorstr_("IP blacklist (2 IPs)"));
                            ImGui::Unindent(10.0f);
                        }
                        
                        ImGui::Spacing();
                        if (ImGui::Button(xorstr_("🔍 Run System Check"), ImVec2(-1, 35))) {
                    ...
                    ImGui::SetCursorPos(ImVec2(562.5, 195));
                    ImGui::MenuChild(xorstr_("AT-Prot-2"), ImVec2(422.5, 470));
                    {
                        ImGui::Spacing();
                        if (ImGui::CollapsingHeader(xorstr_("🚫 Block History"), ImGuiTreeNodeFlags_DefaultOpen)) {
                            ...
                            ImGui::BeginChild(xorstr_("ScrollableHistory"), ImVec2(0, availHeight), true, 
                            ...
                            if (history.empty()) {
                                ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), 
                                                  xorstr_("Waiting for telemetry...\nBlocked requests will appear here."));
                            } else {
                                ...
                                for (size_t i = 0; i < history.size(); i++) {
                                    ...
                                    if (filterStr.length() > 0) {
                                        std::string domainLower = block.domain;
                                        std::transform(domainLower.begin(), domainLower.end(), 
                                                     domainLower.begin(), ::tolower);
                                        
                                        if (domainLower.find(filterStr) == std::string::npos) {
                                            continue;
                                        }
                                    }
                                    ...
                                    ImVec4 color;
                                    if (block.reason.find("Anti-Cheat") != std::string::npos) {
                                        color = ImVec4(1.0f, 0.0f, 0.0f, 1.0f);
                                    } else if (block.reason.find("Telemetry") != std::string::npos || 
                                              block.reason.find("Analytics") != std::string::npos) {
                                        color = ImVec4(1.0f, 0.5f, 0.0f, 1.0f);
                                    } else if (block.reason.find("Firebase") != std::string::npos) {
                                        color = ImVec4(1.0f, 0.0f, 1.0f, 1.0f);
                                    } else if (block.reason.find("Easebar") != std::string::npos) {
                                        color = ImVec4(1.0f, 0.3f, 0.3f, 1.0f);
                                    } else {
                                        color = ImVec4(1.0f, 1.0f, 0.0f, 1.0f);
                                    }
                                    
                                    ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(color.x * 0.3f, color.y * 0.3f, color.z * 0.3f, 0.8f));
                                    ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(color.x * 0.5f, color.y * 0.5f, color.z * 0.5f, 0.8f));
                                    ImGui::PushStyleColor(ImGuiCol_HeaderActive, ImVec4(color.x * 0.7f, color.y * 0.7f, color.z * 0.7f, 0.8f));
                                    
                                    char label[512];
                                    snprintf(label, sizeof(label), "🚫 [%d] %s###block%zu", block.count, block.domain.c_str(), i);
                                    ...
                                    if (ImGui::CollapsingHeader(label)) {
                                        ...
                                        ImGui::Text("🔢 Attempts: %d", block.count);
                                        ImGui::Text("🕐 Last: %s", block.lastBlocked.c_str());
                                ...
                                if (filterStr.length() > 0) {
                                    ...
                                    ImGui::TextColored(ImVec4(0.5f, 0.5f, 1.0f, 1.0f), 
                                                     "Showing %d of %d", visibleCount, (int)history.size());
                                }
                            }
                ...
                // ========== SUB-TAB 2: STATISTICS ==========
                    ...
                    ImGui::SetCursorPos(ImVec2(130, 195));
                    ImGui::MenuChild(xorstr_("AT-Stats"), ImVec2(855, 470));
                    {
                        ImGui::Spacing();
                        ImGui::TextColored(ImVec4(0.0f, 1.0f, 1.0f, 1.0f), xorstr_("📊 Detailed Statistics"));
                        ImGui::Separator();
                        
                        int totalBlocked = globalAntiTelemetry->getTotalBlocked();
                        int uniqueBlocked = (int)globalAntiTelemetry->getBlockHistory().size();
                        
                        ImGui::Text("Total Requests Blocked: %d", totalBlocked);
                        ImGui::Text("Unique Domains Blocked: %d", uniqueBlocked);
                        ImGui::Spacing();
                        
                        ImGui::Separator();
                        ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f), xorstr_("🚫 Block History:"));
                        
                        ImGui::BeginChild(xorstr_("StatsHistory"), ImVec2(0, 350), true);
                        auto history = globalAntiTelemetry->getBlockHistory();
                        
                        if (history.empty()) {
                            ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), 
                                              xorstr_("No blocks recorded yet..."));
                        } else {
                            for (size_t i = 0; i < history.size(); i++) {
                                const auto& block = history[i];
                                ImGui::Text("[%d] %s", (int)i+1, block.domain.c_str());
                                ImGui::Indent(15.0f);
                                ImGui::Text("Reason: %s", block.reason.c_str());
                                ImGui::Text("Count: %d times", block.count);
                                ImGui::Text("Last: %s", block.lastBlocked.c_str());
                                ImGui::Unindent(15.0f);
                                ImGui::Separator();
                            }
                ...
                // ========== SUB-TAB 3: SETTINGS ==========
                    ...
                    ImGui::SetCursorPos(ImVec2(130, 195));
                    ImGui::MenuChild(xorstr_("AT-Settings"), ImVec2(855, 470));
                    {
                        ImGui::Spacing();
                        ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), xorstr_("⚙️ Protection Settings"));
                        ImGui::Separator();
                        ...
                        ImGui::Separator();
                        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.5f, 0.0f, 1.0f));
                        ImGui::TextWrapped(xorstr_("⚠️ Keep ENABLED for maximum protection"));
                        ImGui::SameLine();
                        ImGui::TextWrapped(xorstr_("💡 Check logcat for real-time blocking"));
                        ImGui::PopStyleColor();
                    }
                    ImGui::EndChild();
                }
